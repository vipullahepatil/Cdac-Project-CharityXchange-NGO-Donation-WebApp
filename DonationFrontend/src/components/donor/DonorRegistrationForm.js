...
export function DonorRegistrationForm() {
    ...
    if (!validate()) {
      ...
      if (response.status === 200) {
        if(response.data != "email id present"){
          ...
          setDonorEmailError("Email already registered. Please login or use different email for registration");
        }
      }
    }
  };
  ...
  const validate = () => {
    ...
    if (donorDetails.donorName.trim() === "") {
      setDonorNameError("Name is required");
      isValid = false;
    } else if (!/^[a-zA-Z\s]+$/.test(donorDetails.donorName)) {
      setDonorNameError("Name can only contain letters");
      isValid = false;
    } else {
      setDonorNameError("");
    }
    ...
    if (donorDetails.donorPhone.trim() === "") {
      setDonorPhoneError("Phone number is required");
      isValid = false;
    } else if (!/^[6-9]{1}[0-9]{9}$/.test(donorDetails.donorPhone)) {
      setDonorPhoneError(
        "Phone number can only contain numbers and of 10 digits and first digit(6-9) "
      );
      isValid = false;
    } else {
      setDonorPhoneError("");
    ...
    if (donorDetails.donorEmail.trim() === "") {
      setDonorEmailError("Email ID is required");
      isValid = false;
    ...
    if (!donorDetails.donorAddress.trim()) {
      setDonorAddressError("Address is required");
      isValid = false;
    } else {
    ...
    if (!donorDetails.donorZipCode.trim()) {
      setDonorZipCodeError("ZIP code is required");
      isValid = false;
    ...
    if (donorDetails.donorConfirmPassword.trim() === "") {
      setConfirmDonorPasswordError("Confirm Password is required");
      isValid = false;
    } else if (
